   ...
   public List<chat> listarchat(){
    ...
    public chat listarunchat(int idChat) {
        ...
        try {
            ...
            System.err.println("Error al Buscar por idChat: " + e);
        } finally {
            try {
                if (rs != null) rs.close();
                if (ps != null) ps.close();
...
public boolean actualizar(chat cht) {
    String sql = "UPDATE chatonline SET clienteChat=?, Mensajeria=?, estadoChat=? WHERE idChat=?";
        try {
            conexion = conectar.getConnection();
            ps = conexion.prepareStatement(sql);
            ps.setString(1, cht.getClienteChat());
            ...
            // CORRECCIÓN 2: Agregamos el parámetro obligatorio número 4 para el WHERE de la consulta
            ps.setInt(4, cht.getIdChat());
...
@Override
    public boolean borrar(int idChat) {
        // CORRECCIÓN 3: Cambiada la tabla a 'chatonline' y la condición con el marcador seguro (?)
        String sql = "DELETE FROM chatonline WHERE idChat = ?";
        try {
            conexion = conectar.getConnection();
            ps = conexion.prepareStatement(sql);
            
            // Asigna el parámetro de forma segura
            ps.setInt(1, idChat);
